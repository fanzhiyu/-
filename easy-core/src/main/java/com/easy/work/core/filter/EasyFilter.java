package com.easy.work.core.filter;
...
public class EasyFilter implements Filter {
    ...
    private final Logger logger = LoggerFactory.getLogger(EasyFilter.class);
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException,ServletException {
        ...
        response.setHeader("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept, token, refreshToken");
        ...
        if(!filter){
            ...
            if(!filter){
                ...
                if(StringUtils.isNull(token)){
                    json.put("code", EasyConstant.TOKEN_NULL);
                    json.put("msg", Manager.getMessage(EasyMessage.EASY1006));
                }else{
                    try{
                        ...
                        if(StringUtils.isNotNull(refreshToken)){
                            try {
                                ...
                                String refToken = jwtUtil.createJWT(EasyConstant.JWT_ID, subject);
                                String refresh = jwtUtil.createJWT(EasyConstant.JWT_ID,subject, EasyConstant.JWT_REFRESH_TTL);
                        ...
                        json.put("code", EasyConstant.TOKEN_INVALID);
                        json.put("msg", Manager.getMessage(EasyMessage.EASY1005));
                    } catch (MalformedJwtException e){
                        json.put("code", EasyConstant.TOKEN_ERROR);
                        json.put("msg", Manager.getMessage(EasyMessage.EASY1005));
                    }
                }
